...
0. Прочитай `../brain/README.md` — второй мозг (бизнес-знание: команда, цифры, решения, уроки).
   Если путь не находится — мозг лежит в `~/Desktop/КОД/КЛАУД/brain/`. Из worktree путь тот же.
   Пополняй его по ходу работы: новый факт о бизнесе/людях, принятое решение, урок,
   изменившаяся цифра — пишутся в `brain/`, а не только в `SESSION.md`.
...
5. После этого сообщи одной строкой: на чём остановились и что делаем дальше
