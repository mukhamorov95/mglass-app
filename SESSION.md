## Текущая задача
Procurement kanban восстановлен. Следующий шаг — уточнить что нужно доделать в Procurement.



### feat(procurement): restore material items block — ЗАКРЫТО (8 июня 2026, коммит `9dd5659`)

**Файл:** только `app/admin/procurement/page.tsx`

#### Что было сломано

- Коммит `26125fa` добавил таблицу материалов в detail modal закупки — страница стала падать
- Коммит `ac10577` добавил guard-функции — страница всё равно падала
- Коммит `e7f93ef` — временно убрал весь блок чтобы восстановить страницу

#### Что исправлено

- Вынесен отдельный компонент `MaterialItemsBlock({ items })` — изолирован от основного рендера
- `PurchaseItem` type: все поля кроме `material_name` — опциональные/nullable (защита от любых JSONB-данных)
- `safeNum(v, digits)` и `safeMoney(v)` принимают `unknown` → не упадут при `null`/`undefined`/`string`
- Нет IIFE-паттерна в JSX — вынесено в компонент (это могло быть причиной краша в предыдущих попытках)
- TypeScript: ошибок в файле нет
