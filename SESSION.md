...
### #2, #4 — СЛЕДУЮЩИЕ
- #3 Борд начальника «заказ×этапы»: единый экран (строки=заказы, колонки=стадии, ячейки done/in-progress/problem + кто над чем работает). Супервайзер (`/production-app/supervisor`) сейчас НЕ в навигации и только admin/ceo.
- #4 Кабинет партнёра: нет роли партнёра (роль buyer=снабженец), нет привязки b2b_clients к auth-логину, нет клиент-facing экранов, статус в JSON notes (для realtime вынести в колонку). Ступени: вход+RLS → калькулятор → история → заявка с PDF → статус realtime → (будущее) ИИ-сверка чертежа.
