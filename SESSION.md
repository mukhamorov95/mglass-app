## ДУШЕВАЯ БЮДЖЕТ: РЕДИЗАЙН ПО berusteklo — Ф1+Ф2 (2026-07-03, последнее)
Анализ berusteklo.ru + план сделаны. Владелец выбрал источник картинок = «свои иллюстрации».
Сделано (`ba4767b`, прод, проверено Chrome):
- Убран «Быстрый старт» (пресеты) со страницы душевой.
- Все 12 моделей рисуются единым SVG-набором `components/ShowerModelIcon` (прямые — фронт, угловые — план, дуги/стрелки). Раньше калькулятор предпочитал битые статические jpg (лежали лишь M1/M10/M11) → «стрёмно».
- Статические image_url убраны из SHOWER_MODELS; фото модели теперь из админки `/admin/shower-images` (таблица shower_model_images, GET открыт) — если загружено, иначе SVG. Связь админка↔калькулятор восстановлена (M3 уже показывает загруженное фото).
СЛЕДУЮЩИЕ ФАЗЫ (ждут гоу владельца):
- Ф3: свотчи стекла (`shower_glass_types`) и цвета фурнитуры (`shower_hardware_colors`) — таблицы + админка + UI-свотчи с текстурами/фото (как у berusteklo).
- Ф4: клиентский вид (бейджи «стекло 8мм закалённое / фурнитура AISI 304 / гарантия», живая цена «Итого без скидки → скидка → итого», тумблеры Монтаж/Доставка, «допы по желанию»).
- Ф5: перенос моделей из кода `lib/showerCalculator.ts` в таблицу `shower_models` (админ-редактируемые модели/цены/фото без кода).
