...
- ФИКС КОДА (app/b2b-orders/page.tsx): (1) выборка теперь ПОСТРАНИЧНАЯ (fetchAllOrders, order DESC) — тянет все 2612; (2) месяцы сортируются новые→старые; (3) добавлен уровень ГОД (yearGroups, expandedYears/toggleYear) — год→месяц→заказы; (4) по умолчанию раскрыты последние 3 месяца и их год, остальное свёрнуто. tsc/build зелёные (2 eslint-warn предсуществующие).
