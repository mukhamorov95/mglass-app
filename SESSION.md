## Текущая задача
Система флажков квалификации заявок Авито (Фаза 1). Ветка `feature/avito-lead-flags`.
...
## Следующий шаг
1. ⚠️ Применить миграцию 20260803_crm_lead_flags.sql в Supabase ДО деплоя (webhook пишет новые колонки).
2. Задеплоить ветку, проверить на реальном чате: бот собирает флаги → карточка показывает готовность → при сборе ядра лид 🟢 + задача + Telegram.
3. Фаза 2: доска /crm — фильтр/сортировка по heat, лейн «Готов менеджеру», кнопка «Посчитать»; классификатор импорта отдаёт flags{}.
4. Фаза 3: обучение бота на ответах менеджеров (scripts/mine-manager-replies.mjs + ai_manager_examples + few-shot).
