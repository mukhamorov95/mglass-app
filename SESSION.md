...
## Следующий шаг
1. Смёрджить PR #122, задеплоить (миграции уже в проде).
2. Чтобы обучение работало непрерывно: либо cron-роут для регулярного майнинга, либо live-запись примера при перехвате в /api/avito/thread (МЕНЕДЖЕР пишет → сразу в ai_manager_examples). Решить с владельцем.
3. Открытый вопрос: авто-распределитель 🟢-заявок (round-robin) — пока НЕ делаем, общий пул (решение владельца при малом потоке).
