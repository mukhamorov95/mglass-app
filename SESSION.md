...
### #2–#4 — СЛЕДУЮЩИЕ (не начаты)
- #2 Рассинхрон производства: свести notes.detail_stages (JSON) и production_tasks в один источник (сейчас мост односторонний — отметка с orders/[id] и /p/o не обновляет production_tasks); оживить «Криволинейку» (shape:'curved' нигде не проставляется, этап мёртв); кнопка «взял в работу» (action:'start' есть в API, но не вызывается — WIP не виден); размеры детали на карточках рабочего.
