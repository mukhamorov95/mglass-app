...
export default function MirrorLightingPage() {
  ...
  const inp = 'w-full border border-line rounded-md px-2 py-1.5 text-sm focus:outline-none focus:border-blue-400'
  const num = 'w-full border border-line rounded-md px-2 py-1.5 text-sm font-mono focus:outline-none focus:border-blue-400'

  const currentTabLabel = allTabs.find(t => t.value === tab)?.label ?? tab

  if (loading) return <div className="p-6 text-sm text-muted">Загрузка...</div>

  return (
    <div className="max-w-4xl mx-auto px-4 py-4">

      <PageHeader
        title="Компоненты подсветки зеркал"
        actions={
          <button
            ...
            className="px-3 py-1.5 bg-ink text-white rounded-lg text-sm font-medium hover:bg-[#2a2a28] transition-colors">
            + Добавить
          </button>
        }
      />
      ...
      <div className="flex items-center gap-1 flex-wrap mb-4">
        ...
        {addingTab ? (
          <div className="flex items-center gap-1.5 bg-surface border border-blue-400 rounded-lg px-2 py-1">
            <input
              ref={newTabRef}
              value={newTabName}
            ...
            <button
              ...
              className="text-[11px] font-semibold text-white bg-ink px-2 py-0.5 rounded-md hover:bg-[#2a2a28] disabled:opacity-40 transition-colors whitespace-nowrap">
              {savingTab ? '...' : 'Сохранить'}
            </button>
            <button onClick={() => { setAddingTab(false); setNewTabName('') }}
              className="text-faint hover:text-ink-soft text-base leading-none">×</button>
          </div>
        ) : (
          <button
            onClick={() => setAddingTab(true)}
            className="w-8 h-8 flex items-center justify-center rounded-lg border-2 border-dashed border-faint text-muted hover:border-ink hover:text-ink transition-colors text-base font-medium">
            +
          </button>
        )}
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-line-soft bg-subtle">
              <th className="text-left px-3 py-2 text-[11px] font-semibold text-muted uppercase tracking-wide">Название</th>
              {tab === 'led_strip' && <>
                <th className="text-right px-3 py-2 text-[11px] font-semibold text-muted uppercase tracking-wide">Напряжение</th>
                <th className="text-right px-3 py-2 text-[11px] font-semibold text-muted uppercase tracking-wide">Темп. K</th>
                <th className="text-right px-3 py-2 text-[11px] font-semibold text-muted uppercase tracking-wide">Вт/м</th>
              </>}
              {tab === 'power_supply' && <>
                <th className="text-right px-3 py-2 text-[11px] font-semibold text-muted uppercase tracking-wide">Напряжение</th>
                <th className="text-right px-3 py-2 text-[11px] font-semibold text-muted uppercase tracking-wide">Макс. Вт</th>
              </>}
              <th className="text-right px-3 py-2 text-[11px] font-semibold text-muted uppercase tracking-wide">Ед.</th>
              <th className="text-right px-3 py-2 text-[11px] font-semibold text-muted uppercase tracking-wide">Себест.</th>
              <th className="text-right px-3 py-2 text-[11px] font-semibold text-muted uppercase tracking-wide">Продажная</th>
              <th className="px-3 py-2" />
            </tr>
          </thead>
          <tbody>
            {visible.length === 0 && (
              <tr><td colSpan={10} className="px-3 py-8 text-center text-sm text-muted">
                Нет позиций — нажмите «+ Добавить»
              </td></tr>
            )}
            {visible.map(item => (
              <tr key={item.id} className={`border-b border-canvas last:border-0 hover:bg-subtle ${!item.active ? 'opacity-40' : ''}`}>
                <td className="px-3 py-2">
                  <p className="text-sm font-medium text-ink">{item.name}</p>
                  {item.short_name && <p className="text-[11px] text-violet-500">→ {item.short_name}</p>}
                  {item.description && <p className="text-[11px] text-muted">{item.description}</p>}
                </td>
                {tab === 'led_strip' && <>
                  <td className="px-3 py-2 text-right text-sm font-mono tabular-nums">{item.voltage ? `${item.voltage}V` : '—'}</td>
                  <td className="px-3 py-2 text-right text-sm font-mono tabular-nums">{fmtN(item.color_temp)}</td>
                  <td className="px-3 py-2 text-right text-sm font-mono tabular-nums">{item.power_per_meter ?? '—'}</td>
                </>}
                {tab === 'power_supply' && <>
                  <td className="px-3 py-2 text-right text-sm font-mono tabular-nums">{item.voltage ? `${item.voltage}V` : '—'}</td>
                  <td className="px-3 py-2 text-right text-sm font-mono tabular-nums">{fmtN(item.max_power)}</td>
                </>}
                <td className="px-3 py-2 text-right text-[11px] text-muted">{item.unit}</td>
                <td className="px-3 py-2 text-right text-sm font-mono tabular-nums">{fmtN(item.cost_price)} ₽</td>
                <td className="px-3 py-2 text-right text-sm font-mono tabular-nums text-muted">
                ...
                <td className="px-3 py-2">
                  <div className="flex items-center gap-1 justify-end">
                    ...
                    <button onClick={() => { setSaveError(null); setEditing({ ...item }) }}
                      className="text-[11px] px-2 py-0.5 rounded border border-line text-ink-soft hover:bg-canvas transition-colors">
                      Ред.
                    </button>
      ...
      {editing && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4">
          <div className="bg-surface rounded-xl shadow-xl w-full max-w-md p-5">
            ...
            <div className="space-y-3">
              <div>
                <label className="text-[11px] text-muted font-semibold uppercase tracking-wide">Название (от поставщика) *</label>
                <input className={inp} value={editing.name ?? ''} onChange={e => setEditing(p => ({ ...p!, name: e.target.value }))} />
              </div>

              <div>
                <label className="text-[11px] text-muted font-semibold uppercase tracking-wide">
                  Короткое название
                  <span className="ml-1 font-normal normal-case text-faint">— показывается в калькуляторе</span>
                </label>
              ...
              <div>
                <label className="text-[11px] text-muted font-semibold uppercase tracking-wide">Описание</label>
              ...
              {(tab === 'led_strip' || tab === 'power_supply') && (
                <div>
                  <label className="text-[11px] text-muted font-semibold uppercase tracking-wide">Напряжение (V)</label>
                    ...
                    {[12, 24].map(v => (
                      <button key={v} onClick={() => setEditing(p => ({ ...p!, voltage: editing.voltage === v ? null : v }))}
                        className={`flex-1 py-1.5 rounded border text-sm font-medium transition-colors ${
                          editing.voltage === v ? 'bg-ink text-white border-ink' : 'border-line text-ink-soft hover:bg-canvas'
                        }`}>
                        {v}V
                      </button>
                    ))}
              ...
              {tab === 'led_strip' && (
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[11px] text-muted font-semibold uppercase tracking-wide">Цветовая температура (K)</label>
                  ...
                  <div>
                    <label className="text-[11px] text-muted font-semibold uppercase tracking-wide">Мощность (Вт/м)</label>
              ...
              {tab === 'power_supply' && (
                <div>
                  <label className="text-[11px] text-muted font-semibold uppercase tracking-wide">Макс. мощность (Вт)</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] text-muted font-semibold uppercase tracking-wide">Себестоимость (₽)</label>
                  <input type="number" className={num} value={editing.cost_price ?? 0}
                    onChange={e => setEditing(p => ({ ...p!, cost_price: Number(e.target.value) || 0 }))} />
                </div>
                <div>
                  <label className="text-[11px] text-muted font-semibold uppercase tracking-wide">Продажная (₽)</label>
              ...
              <div>
                <label className="text-[11px] text-muted font-semibold uppercase tracking-wide">Единица</label>
            ...
            <div className="flex gap-2 mt-5">
              <button onClick={save} disabled={saving || !editing.name?.trim()}
                className="flex-1 py-2 bg-ink text-white rounded-lg text-sm font-semibold hover:bg-[#2a2a28] disabled:opacity-50 transition-colors">
                {saving ? 'Сохранение...' : 'Сохранить'}
              </button>
              <button onClick={() => setEditing(null)}
                className="px-4 py-2 border border-line rounded-lg text-sm text-ink-soft hover:bg-subtle transition-colors">
                Отмена
              </button>
            </div>
          </div>
