...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="mb-10">
      <h2 className="text-[16px] font-bold text-[#111110] mb-4 pb-2 border-b border-[#e4e4e0]">{title}</h2>
      {children}
    </section>
  )
}

function Card({ children, color = 'border-[#e4e4e0]' }: { children: React.ReactNode; color?: string }) {
  return (
    <div className={`bg-white rounded-xl border ${color} p-5 mb-4`}>
      {children}
    </div>
  )
}
...
function Formula({ formula, note }: { formula: string; note?: string }) {
  return (
    <div className="my-3">
      <div className="bg-[#f5f5f3] rounded-lg px-4 py-3 font-mono text-[13px] text-[#111110] border border-[#e4e4e0]">
        {formula}
      </div>
      {note && <p className="text-[12px] text-[#9a9a95] mt-1.5 ml-1">{note}</p>}
    </div>
  )
}
...
function Row({ label, value, color }: { label: string; value: string; color?: string }) {
  return (
    <div className="flex items-center justify-between py-2 border-b border-[#f5f5f3] last:border-0">
      <span className="text-[13px] text-[#6b6b66]">{label}</span>
      <span className={`text-[13px] font-semibold font-mono ${color ?? 'text-[#111110]'}`}>{value}</span>
    </div>
  )
}
  ...
  return (
    <div className="grid grid-cols-3 gap-3 my-4">
      <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 text-center">
        ...
        <p className="text-[11px] font-bold text-emerald-700 uppercase tracking-wider mb-1">Рекомендовано</p>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-center">
        ...
        <p className="text-[11px] font-bold text-amber-700 uppercase tracking-wider mb-1">Допустимо</p>
      ...
      <div className="bg-red-50 border border-red-200 rounded-xl p-4 text-center">
        ...
        <p className="text-[11px] font-bold text-red-700 uppercase tracking-wider mb-1">Ниже минимума</p>
...
export default async function PricingManualPage() {
  ...
  return (
    <div className="max-w-[860px] mx-auto px-6 py-8">
      ...
      {/* Header */}
      <div className="mb-8">
        <p className="text-[11px] font-bold text-[#9a9a95] uppercase tracking-wider mb-1">Только для владельца · Admin</p>
        <h1 className="text-[26px] font-bold text-[#111110] tracking-tight">Руководство по ценообразованию</h1>
      ...
      <Section title="1. Единая формула ценообразования">
        <Card color="border-blue-200">
          <p className="text-[13px] font-semibold text-[#111110] mb-2">Основная формула (для всех продуктов B2C):</p>
          <Formula
          ...
          />
          <p className="text-[13px] font-semibold text-[#111110] mt-4 mb-2">Партнёрская надбавка (grossup):</p>
          <Formula
          ...
          />
          <p className="text-[13px] font-semibold text-[#111110] mt-4 mb-2">Скидка:</p>
          <Formula
            formula="Итоговая цена = Цена с партнёркой × (1 − Скидка%)"
            note="Скидка применяется ПОСЛЕДНЕЙ, уже после партнёрской надбавки."
          />
          <p className="text-[13px] font-semibold text-[#111110] mt-4 mb-2">Реальная прибыль (не margin × price!):</p>
          <Formula
      ...
      <Section title="2. Система контроля маржи (светофор)">
        <p className="text-[13px] text-[#6b6b66] mb-4">
        ...
        <Card>
          <p className="text-[12px] font-bold text-[#9a9a95] uppercase tracking-wider mb-3">Текущие пороги по продуктам</p>
          <div className="overflow-x-auto">
            <table className="w-full text-[13px]">
              <thead>
                <tr className="border-b border-[#e4e4e0]">
                  <th className="text-left py-2 text-[#6b6b66] font-semibold">Продукт</th>
                  <th className="text-center py-2 text-[#6b6b66] font-semibold">Расходы%</th>
                  <th className="text-center py-2 text-[#6b6b66] font-semibold">Целевая маржа</th>
                  <th className="text-center py-2 text-[#6b6b66] font-semibold">Мин. маржа</th>
                  <th className="text-center py-2 text-[#6b6b66] font-semibold">Макс. скидка</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr key={label} className="border-b border-[#f5f5f3] last:border-0">
                    <td className="py-2.5 font-medium text-[#111110]">{label}</td>
      ...
      <Section title="3. Зеркала и зеркала с подсветкой">
        <Card color="border-blue-100">
          <p className="text-[12px] font-bold text-blue-600 uppercase tracking-wider mb-3">Себестоимость — BOM</p>
        ...
        <Card>
          <p className="text-[12px] font-bold text-[#9a9a95] uppercase tracking-wider mb-3">Специальные коэффициенты</p>
          <div className="space-y-1">
          ...
          {mirrorSettings && (
            <div className="mt-4 pt-4 border-t border-[#f5f5f3]">
              <p className="text-[12px] text-[#9a9a95] mb-2">Пример: себестоимость 10 000 ₽ →</p>
              <p className="text-[20px] font-bold font-mono text-[#111110]">
        ...
        <Card color="border-emerald-100">
          <p className="text-[12px] font-bold text-emerald-600 uppercase tracking-wider mb-3">Заработок менеджера — зеркала</p>
          <div className="space-y-1">
      ...
      <Section title="4. Душевые перегородки">
        <Card color="border-cyan-100">
          <p className="text-[12px] font-bold text-cyan-600 uppercase tracking-wider mb-3">Тир-система</p>
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-[#f5f5f3] rounded-lg p-4">
              <p className="text-[13px] font-bold text-[#111110] mb-2">Бюджет</p>
              <div className="space-y-1">
            ...
            <div className="bg-[#f5f5f3] rounded-lg p-4">
              <p className="text-[13px] font-bold text-[#111110] mb-2">Стандарт</p>
              <div className="space-y-1">
        ...
        <Card>
          <p className="text-[12px] font-bold text-[#9a9a95] uppercase tracking-wider mb-3">Коэффициенты цвета фурнитуры</p>
          <div className="space-y-1">
        ...
        <Card>
          <p className="text-[12px] font-bold text-[#9a9a95] uppercase tracking-wider mb-3">BOM — что входит в себестоимость</p>
          <div className="space-y-1">
        ...
        <Card color="border-orange-100">
          <p className="text-[12px] font-bold text-orange-600 uppercase tracking-wider mb-3">Edge cases — нестандартные ситуации</p>
          <div className="space-y-1">
      ...
      <Section title="5. Лофт-перегородки">
        <Card color="border-orange-100">
          <p className="text-[12px] font-bold text-orange-600 uppercase tracking-wider mb-3">Геометрия — как считается площадь и металл</p>
          <Formula
        ...
        <Card>
          <p className="text-[12px] font-bold text-[#9a9a95] uppercase tracking-wider mb-3">BOM лофта</p>
          <div className="space-y-1">
        ...
        {loftSettings && (
          <Card>
            <p className="text-[12px] font-bold text-[#9a9a95] uppercase tracking-wider mb-2">Пример расчёта лофта 2000×2200 мм, 1 секция:</p>
            <div className="space-y-1">
        ...
        <Card color="border-emerald-100">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-emerald-50 rounded-lg p-4 border border-emerald-200">
              <p className="text-[11px] font-bold text-emerald-700 uppercase tracking-wider mb-2">Базовая комиссия</p>
              <p className="text-[22px] font-bold text-emerald-800">2%</p>
            ...
            <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
              <p className="text-[11px] font-bold text-blue-700 uppercase tracking-wider mb-2">Upsell-бонус</p>
              <p className="text-[22px] font-bold text-blue-800">10%</p>
      ...
      <Section title="7. Политика скидок">
        <Card color="border-red-100">
          <div className="grid grid-cols-2 gap-6">
            <div>
              <p className="text-[12px] font-bold text-[#9a9a95] uppercase tracking-wider mb-3">Менеджер может дать</p>
              <div className="space-y-1">
            ...
            <div>
              <p className="text-[12px] font-bold text-[#9a9a95] uppercase tracking-wider mb-3">Требует согласования</p>
              <div className="space-y-1">
        ...
        <Card>
          <p className="text-[12px] font-bold text-[#9a9a95] uppercase tracking-wider mb-3">Когда можно давать скидку</p>
          <div className="space-y-2">
            {[
              ...
              <div key={case_} className="flex gap-3">
                <span className="text-[13px] font-medium text-[#111110] w-64 flex-shrink-0">{case_}</span>
                <span className="text-[13px] text-[#6b6b66]">{rule}</span>
              </div>
            ))}
          </div>
        </Card>
      ...
      <Section title="9. Динамическое ценообразование — архитектура">
        <Card color="border-indigo-100">
          <p className="text-[13px] text-[#6b6b66] mb-4">
            Система готова к подключению динамических коэффициентов. Таблица <code className="text-[12px] bg-[#f5f5f3] px-1 rounded">coefficients</code> уже есть в БД.
          </p>
          <div className="space-y-3">
            {[
              ...
              <div key={trigger} className="flex items-center gap-3">
                ...
                <span className="text-[13px] text-[#6b6b66] w-60 flex-shrink-0">{trigger}</span>
                <span className="text-[13px] font-medium text-[#111110]">{effect}</span>
              </div>
            ))}
          </div>
        </Card>
      ...
      <Section title="10. Чеклист финансового управления">
        <Card color="border-amber-100">
          <p className="text-[12px] font-bold text-amber-700 uppercase tracking-wider mb-4">Ежемесячные проверки для собственника</p>
          <div className="space-y-2">
            {[
              ...
              <div key={i} className="flex items-start gap-2.5">
                ...
                <span className="text-[13px] text-[#6b6b66]">{item}</span>
              </div>
            ))}
          </div>
        </Card>
      ...
      {/* Footer */}
      <div className="mt-8 pt-6 border-t border-[#e4e4e0] text-center">
        <p className="text-[12px] text-[#9a9a95]">
          MGlass Pricing Manual · Документ актуален для текущей версии платформы
        </p>
        <p className="text-[11px] text-[#c4c4be] mt-1">
          Изменить пороги маржи можно в <a href="/admin/settings" className="underline hover:text-[#6b6b66]">Финансовых настройках</a>
        </p>
      </div>
