      ...
      {detail && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setDetail(null)}>
          <div className="bg-white rounded-xl border border-[#e4e4e0] p-6 w-full max-w-2xl max-h-[85vh] overflow-y-auto shadow-xl" onClick={e => e.stopPropagation()}>
            ...
            <MaterialItemsBlock items={detail.items} />
...
function MaterialItemsBlock({ items }: { items?: PurchaseItem[] | null }) {
  const rows: PurchaseItem[] = Array.isArray(items) ? items : []
          ...
          <table className="w-full text-[11px]">
            ...
            <tbody className="divide-y divide-[#f4f4f0]">
              {rows.map((item, idx) => {
                ...
                const sheetsVal = item.sheets_count != null && Number.isFinite(Number(item.sheets_count))
                  ? String(item.sheets_count) : '—'
                return (
                  <tr key={idx} className={`hover:bg-[#fafaf9] ${isUnmatched ? 'bg-amber-50/40' : ''}`}>
                    <td className="px-2.5 py-2 min-w-[120px]">
                      ...
                      {item.category && <p className="text-[9px] text-[#9a9a95]">{String(item.category)}</p>}
                      {isUnmatched && (
                    ...
                    <td className="px-2.5 py-2 text-right font-mono font-semibold text-[#111110] whitespace-nowrap">
                      {safeNum(item.area_m2, 2)}
                    </td>
                    <td className="px-2.5 py-2 text-right font-mono font-semibold text-[#111110]">
                      {sheetsVal}
                    </td>
                    <td className="px-2.5 py-2 text-right font-mono text-[#6b6b66] whitespace-nowrap">
                      {safeNum(item.weight_kg, 1)} кг
                    </td>
                    <td className="px-2.5 py-2 text-right font-mono whitespace-nowrap">
                      {item.estimated_cost != null
                        ? <span className="font-semibold text-[#111110]">{safeMoney(item.estimated_cost)}</span>
                        : <span className="text-[#c4c4be]">—</span>}
                    </td>
                    <td className="px-2.5 py-2 min-w-[80px]">
                      <div className="flex flex-wrap gap-1">
                        {refs.map((ref, i) => (
                          <span key={i} className="text-[9px] bg-blue-50 text-blue-600 px-1.5 py-px rounded whitespace-nowrap">{String(ref)}</span>
                        ))}
                      </div>
                    </td>
                  </tr>
