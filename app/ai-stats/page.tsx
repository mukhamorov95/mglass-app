...
export default function AiStatsPage() {
  ...
  const CARDS = [
    { label: 'Всего диалогов', value: total, sub: 'обработал AI', color: 'text-ink' },
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <PageHeader title="Статистика AI-менеджера" subtitle="Все диалоги Владислава с клиентами" />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8 mt-4">
        {CARDS.map(c => (
          <div key={c.label} className="bg-surface rounded-2xl border border-line p-4">
            <p className="text-[12px] text-ink-soft mb-1">{c.label}</p>
            <p className={`text-[28px] font-semibold leading-none mb-1 tabular-nums ${c.color}`}>{loading ? '—' : c.value}</p>
      ...
      {total > 0 && (
        <div className="bg-surface rounded-2xl border border-line p-4 mb-6">
          <div className="flex items-center justify-between mb-2">
            <p className="text-[13px] font-medium text-ink">Конверсия в замер</p>
            <p className="text-[13px] font-semibold text-emerald-600 tabular-nums">{convRate}%</p>
          </div>
          ...
          <div className="flex gap-4 mt-2">
            <span className="text-[11px] text-ink-soft tabular-nums">✅ Замер: {measurement}</span>
            <span className="text-[11px] text-ink-soft tabular-nums">👤 Менеджер: {human}</span>
            <span className="text-[11px] text-ink-soft tabular-nums">💬 Активно: {active}</span>
          </div>
        </div>
      )}
      ...
      <div className="bg-surface rounded-2xl border border-line overflow-hidden">
        <div className="px-5 py-4 border-b border-line-soft">
          <SectionHeader title="Все диалоги" />
        </div>

        {loading ? (
          <div className="px-5 py-10 text-center text-[13px] text-muted">Загрузка...</div>
        ) : realChats.length === 0 ? (
          <div className="px-5 py-10 text-center text-[13px] text-muted">Диалогов пока нет</div>
        ) : (
          <div className="divide-y divide-line-soft">
            {realChats.map(chat => (
              <button
                ...
                className="w-full px-5 py-4 flex items-start gap-4 hover:bg-subtle transition-colors text-left"
              >
                {/* Avatar */}
                <div className="w-9 h-9 rounded-full bg-line-soft flex items-center justify-center text-[15px] shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {chat.client_name && (
                    <p className="text-[12px] text-muted mb-0.5">{fmtPhone(chat.chat_id)}</p>
                  )}

                  {chat.last_message && (
                    <p className="text-[12px] text-ink-soft truncate">{chat.last_message}…</p>
                  )}

                  <div className="flex items-center gap-3 mt-1">
                    <span className="text-[11px] text-muted">{fmt(chat.created_at)}</span>
                    <span className="text-[11px] text-muted tabular-nums">{chat.message_count} сообщений</span>
                    {chat.amo_lead_id && (
                      <a
      ...
      {selected && (
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="w-full max-w-md bg-surface flex flex-col shadow-2xl">
            {/* Header */}
            <div className="px-5 py-4 border-b border-line flex items-center justify-between">
              <div>
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  ...
                  <span className="text-[11px] text-muted">{fmt(selected.created_at)}</span>
                </div>
              </div>
              <button
                onClick={() => setSelected(null)}
                className="w-8 h-8 rounded-full bg-line-soft flex items-center justify-center text-ink-soft hover:bg-line"
              >
                ✕
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {loadingMsgs ? (
                <p className="text-center text-[13px] text-muted py-10">Загрузка...</p>
              ) : messages.length === 0 ? (
                <p className="text-center text-[13px] text-muted py-10">Нет сообщений</p>
              ) : messages.map((m, i) => (
                <div key={i} className={`flex ${m.role === 'assistant' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] px-3 py-2 rounded-2xl text-[13px] leading-relaxed whitespace-pre-wrap
                    ${m.role === 'assistant'
                      ...
                      : 'bg-line-soft text-ink rounded-bl-sm'
                    }`}>
                    {m.content}
                  </div>
                </div>
