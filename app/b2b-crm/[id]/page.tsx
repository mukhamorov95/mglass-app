...
const statusMeta   = (v: string | null) => B2B_STATUSES.find(s => s.value === v) ?? { label: '—', color: 'bg-line-soft text-ink-soft' }
const scoreMeta    = (v: string | null) => B2B_SCORES.find(s => s.value === v) ?? { label: '—', color: 'bg-line-soft text-ink-soft' }
...
export default function B2BClientCardPage() {
  ...
  if (!client || !crm) return (
    <div className="min-h-screen flex items-center justify-center text-[13px] text-muted">Клиент не найден</div>
  )
  ...
  return (
    <div className="min-h-screen bg-canvas">
      {toast && (
        <div className="fixed top-4 right-4 z-50 bg-ink text-white text-[12px] px-4 py-2.5 rounded-xl shadow-lg max-w-sm">
          {toast}
        </div>
      )}
      <div className="max-w-[1100px] mx-auto px-4 py-5">
        ...
        <div className="flex items-center gap-2 mb-4 text-[12px] text-muted">
          <Link href="/b2b-crm" className="hover:text-ink transition-colors">B2B CRM</Link>
          <span>/</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-4 items-start">
          ...
          <div className="space-y-3">
              ...
              <div className="flex items-start justify-between mb-3">
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className={`text-[11px] font-semibold px-1.5 py-0.5 rounded ${scm.color}`}>{scm.label}</span>
                    <span className={`text-[11px] font-medium px-2 py-0.5 rounded-full ${sm.color}`}>{sm.label}</span>
                  </div>
                  <h2 className="text-[17px] font-semibold text-ink mt-1.5 leading-snug">{client.name}</h2>
                  {crm.segment && (
                    <p className="text-[12px] text-muted mt-0.5">{segmentLabel(crm.segment)}</p>
                  )}
                </div>
                <button
                  ...
                  className="text-[11px] font-medium text-ink-soft border border-line px-2.5 py-1 rounded-lg hover:bg-canvas transition-colors shrink-0">
              ...
              {!editing ? (
                <div className="space-y-2 text-[13px]">
                  {client.contact && (
                    <div className="flex gap-2">
                      <span className="text-muted w-24 shrink-0">Контакт</span>
                      <span className="text-ink font-medium">{client.contact}</span>
                    </div>
                  )}
                  {client.phone && (
                    <div className="flex gap-2">
                      <span className="text-muted w-24 shrink-0">Телефон</span>
                      <a href={`tel:${client.phone}`} className="text-blue-600 font-mono hover:underline">{client.phone}</a>
                    </div>
                  )}
                  {crm.city && (
                    <div className="flex gap-2">
                      <span className="text-muted w-24 shrink-0">Город</span>
                      <span className="text-ink">{crm.city}</span>
                    </div>
                  )}
                  {crm.manager_name && (
                    <div className="flex gap-2">
                      <span className="text-muted w-24 shrink-0">Менеджер</span>
                      <span className="text-ink">{crm.manager_name}</span>
                    </div>
                  )}
                  {client.discount_percent > 0 && (
                    <div className="flex gap-2">
                      <span className="text-muted w-24 shrink-0">Скидка</span>
                      <span className="text-emerald-600 font-semibold">{client.discount_percent}%</span>
                    </div>
                  )}
                  {crm.next_contact_date && (
                    <div className="flex gap-2">
                      <span className="text-muted w-24 shrink-0">Следующий контакт</span>
                      <span className="text-ink">{fmtDate(crm.next_contact_date)}</span>
                    </div>
                  )}
                  {crm.crm_notes && (
                    <div className="pt-2 border-t border-line-soft text-[12px] text-ink-soft leading-relaxed">
                      {crm.crm_notes}
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-2.5">
                  {[
                    ...
                    <div key={f.key}>
                      <label className="block text-[10px] font-semibold text-muted uppercase tracking-widest mb-1">{f.label}</label>
                      <input type={f.type}
                        className="w-full bg-canvas border border-line rounded-lg px-3 py-1.5 text-[13px] text-ink outline-none focus:border-ink"
                  ...
                  <div>
                    <label className="block text-[10px] font-semibold text-muted uppercase tracking-widest mb-1">Сегмент</label>
                    <select className="w-full bg-canvas border border-line rounded-lg px-3 py-1.5 text-[13px] text-ink outline-none focus:border-ink"
                  ...
                  <div>
                    <label className="block text-[10px] font-semibold text-muted uppercase tracking-widest mb-1">Статус</label>
                    <select className="w-full bg-canvas border border-line rounded-lg px-3 py-1.5 text-[13px] text-ink outline-none focus:border-ink"
                  ...
                  <div>
                    <label className="block text-[10px] font-semibold text-muted uppercase tracking-widest mb-1">Класс (A/B/C)</label>
                    <div className="flex gap-1.5">
                      {B2B_SCORES.map(s => (
                        <button key={s.value} onClick={() => setEditCRM(p => ({ ...p, score: s.value }))}
                          className={`flex-1 py-1.5 rounded-lg text-[13px] font-semibold transition-colors ${editCRM.score === s.value ? s.color : 'bg-line-soft text-muted'}`}>
                          {s.value}
                        </button>
                      ))}
                    </div>
                  ...
                  <div>
                    <label className="block text-[10px] font-semibold text-muted uppercase tracking-widest mb-1">Заметка</label>
                    <textarea rows={2}
                      className="w-full bg-canvas border border-line rounded-lg px-3 py-1.5 text-[13px] text-ink outline-none focus:border-ink resize-none"
                  ...
                  <button onClick={saveClient} disabled={savingClient || !editCRM.name?.trim()}
                    className="w-full bg-ink text-white text-[13px] font-semibold py-2 rounded-lg hover:bg-[#2a2a28] disabled:opacity-40 transition-colors">
            ...
            {orders.length > 0 && (
              <div className="bg-surface border border-line rounded-xl p-4">
                <p className="text-[10px] font-semibold text-muted uppercase tracking-widest mb-3">Заказы в текущем году</p>
                <div className="flex gap-4 mb-3">
                  <div>
                    <p className="text-[20px] font-semibold text-ink font-mono leading-none tabular-nums">{fmt(yearTotal)}</p>
                    <p className="text-[11px] text-muted mt-0.5">{orders.length} просчётов</p>
                  </div>
                </div>
                <div className="space-y-1.5 max-h-[200px] overflow-y-auto">
                  {orders.map(o => {
                    ...
                    return (
                      <div key={o.id} className="flex items-center gap-2 text-[12px] py-1.5 border-b border-canvas last:border-0 group">
                        <Link href="/b2b-quotes" className="flex-1 flex items-center gap-2 hover:text-blue-600 transition-colors min-w-0">
                          <span className="text-muted shrink-0">{fmtDateShort(o.created_at)}</span>
                          <span className="font-mono font-semibold text-ink">{fmt(total)}</span>
                          {o.margin_percent > 0 && (
                            <span className="text-[10px] text-muted">{o.margin_percent}% маржа</span>
                          )}
                        </Link>
            ...
            <div className="bg-surface border border-line rounded-xl p-4">
              ...
              <div className="space-y-1.5">
                <Link href={`/calculator/b2b?client=${clientId}`}
                  className="flex items-center gap-2 px-3 py-2 rounded-lg border border-line text-[13px] text-ink hover:bg-canvas transition-colors">
            ...
            <div className="bg-surface border border-line rounded-xl overflow-hidden">
              <div className="px-4 py-3 flex items-center justify-between border-b border-line-soft">
                <p className="text-[10px] font-semibold text-muted uppercase tracking-widest">AI-анализ</p>
                <button
                  onClick={runAnalysis}
                  disabled={aiLoading}
                  className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[12px] font-semibold bg-ink text-white hover:bg-[#2a2a28] disabled:opacity-50 transition-colors">
                  {aiLoading ? (
                ...
                <div className="divide-y divide-line-soft">
                  ...
                  <div className="px-4 py-3 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] text-ink-soft mt-0.5 leading-snug">{aiAnalysis.risk_reason}</p>
                    </div>
                  </div>

                  {/* Summary */}
                  <div className="px-4 py-3">
                    <p className="text-[10px] font-semibold text-muted uppercase tracking-widest mb-1">Оценка</p>
                    <p className="text-[12px] text-ink leading-snug">{aiAnalysis.summary}</p>
                  </div>

                  {/* Next step */}
                  <div className="px-4 py-3">
                    <p className="text-[10px] font-semibold text-muted uppercase tracking-widest mb-1">Следующий шаг</p>
                    <p className="text-[12px] text-ink leading-snug">{aiAnalysis.next_step}</p>
                  ...
                  {aiAnalysis.message_template && (
                    <div className="px-4 py-3">
                      <div className="flex items-center justify-between mb-1.5">
                        <p className="text-[10px] font-semibold text-muted uppercase tracking-widest">Шаблон сообщения</p>
                        <div className="flex gap-1.5">
                          <button
                            onClick={() => {
                      ...
                      <p className="text-[12px] text-ink-soft leading-relaxed whitespace-pre-wrap bg-canvas rounded-lg p-2.5">
                        {aiAnalysis.message_template}
                      </p>
                    </div>
                  )}
          ...
          <div className="space-y-3">
            ...
            <div className="bg-surface border border-line rounded-xl p-5">
              <p className="text-[10px] font-semibold text-muted uppercase tracking-widest mb-3">Добавить взаимодействие</p>

              <div className="flex gap-1.5 flex-wrap mb-3">
                {B2B_INTERACTION_TYPES.map(t => (
                  <button key={t.value} onClick={() => setNewInt(p => ({ ...p, type: t.value }))}
                    className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-[12px] font-medium transition-colors ${newInt.type === t.value ? 'bg-ink text-white' : 'bg-line-soft text-ink-soft hover:bg-line-soft'}`}>
              ...
              <textarea rows={2} placeholder="Что произошло? О чём говорили?"
                className="w-full bg-canvas border border-line rounded-lg px-3 py-2 text-[13px] text-ink outline-none focus:border-ink resize-none mb-2"
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2">
                  <label className="text-[11px] text-muted">Следующий контакт:</label>
                  <input type="date"
                    className="bg-canvas border border-line rounded-lg px-2 py-1.5 text-[12px] text-ink outline-none focus:border-ink"
                ...
                <button onClick={saveInteraction} disabled={savingInt || !newInt.note.trim()}
                  className="ml-auto bg-ink text-white text-[13px] font-semibold px-4 py-1.5 rounded-lg hover:bg-[#2a2a28] disabled:opacity-40 transition-colors">
            ...
            {templates.length > 0 && (
              <div className="bg-surface border border-line rounded-xl overflow-hidden">
                <button
                  ...
                  <svg className={`w-3.5 h-3.5 text-muted transition-transform duration-150 ${showTemplates ? 'rotate-180' : ''}`}
                ...
                {showTemplates && (
                  <div className="border-t border-line-soft divide-y divide-line-soft">
                    {templates.map(t => (
                      <div key={t.id} className="px-5 py-4 hover:bg-subtle transition-colors group">
                        <div className="flex items-start justify-between gap-2 mb-1.5">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] font-medium bg-line-soft text-ink-soft px-1.5 py-0.5 rounded">
                          ...
                          <div className="flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                            ...
                            <button onClick={() => insertTemplate(t)}
                              className="text-[10px] font-medium px-2 py-1 rounded-lg bg-ink text-white hover:bg-[#2a2a28] transition-colors">
                              В заметку
                            </button>
                            <button onClick={() => copyTemplate(t)}
                              className={`text-[10px] font-medium px-2 py-1 rounded-lg border transition-colors ${copiedId === t.id ? 'border-emerald-300 bg-emerald-50 text-emerald-700' : 'border-line text-ink-soft hover:bg-line-soft'}`}>
                              {copiedId === t.id ? '✓ Скопировано' : 'Копировать'}
                            </button>
                          </div>
                        </div>
                        <p className="text-[12px] text-ink-soft leading-relaxed whitespace-pre-wrap">{t.body}</p>
                        ...
                        {personalizedPreview?.templateId === t.id && (
                          <div className="mt-3 rounded-xl border border-purple-200 bg-purple-50 overflow-hidden">
                            <div className="px-3 py-2 border-b border-purple-200 flex items-center justify-between">
                              ...
                              <button onClick={() => setPersonalizedPreview(null)} className="text-muted hover:text-red-400 transition-colors text-lg leading-none">×</button>
                            </div>
                            <div className="p-3">
                              <p className="text-[12px] text-ink leading-relaxed whitespace-pre-wrap">{personalizedPreview.personalized}</p>
                            </div>
                            <div className="px-3 py-2 border-t border-purple-200 flex gap-2">
                              <button
                                ...
                                className="text-[11px] font-semibold bg-ink text-white px-3 py-1 rounded-lg hover:bg-[#2a2a28] transition-colors">
                                В заметку
                              </button>
                              <button
                                onClick={() => {
            ...
            <div className="bg-surface border border-line rounded-xl overflow-hidden">
              ...
              {interactions.length === 0 ? (
                ...
                <div className="divide-y divide-line-soft">
                  {interactions.map(i => (
                    <div key={i.id} className="px-5 py-4 hover:bg-subtle transition-colors group">
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex items-start gap-2.5 flex-1 min-w-0">
                          <span className="text-[16px] mt-0.5 shrink-0">{typeIcon(i.type)}</span>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-0.5">
                              <span className="text-[11px] font-semibold text-muted">{typeLabel(i.type)}</span>
                              <span className="text-[11px] text-faint">
                                {fmtDate(i.created_at)} · {fmtTime(i.created_at)}
                              </span>
                              {i.created_by && (
                                <span className="text-[11px] text-faint">{i.created_by}</span>
                              )}
                            </div>
                            <p className="text-[13px] text-ink leading-snug">{i.note}</p>
                            {i.outcome && (
                        ...
                        <button onClick={() => deleteInteraction(i.id)}
                          className="opacity-0 group-hover:opacity-100 text-faint hover:text-red-400 transition-all text-lg leading-none shrink-0">
                          ×
                        </button>
                      </div>
                    </div>
