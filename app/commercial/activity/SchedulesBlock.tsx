...
type Draft = { starts_on: string; work_from: string; work_to: string; work_days: number[]; note: string }
...
function Row({ userId, name, s, onSaved }: { userId: number; name: string; s?: ManagerSchedule; onSaved: () => void }) {
  const [d, setD] = useState<Draft>({
    ...
    work_days: s?.work_days ?? [1, 2, 3, 4, 5], note: s?.note ?? '',
  })
  ...
  return (
    <tr className="border-b border-[#f0f0ec] align-top">
      <td className="py-2 pr-2 font-medium text-[#111110] text-[13px]">{name}</td>
