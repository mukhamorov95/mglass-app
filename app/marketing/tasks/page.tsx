...
const PRIORITIES = [
  ...
  { v: 'low',    l: '⬇️ Низкий',  cls: 'bg-[#f5f5f0] text-[#8a8a85]' },
]
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-8">
      ...
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-[22px] font-bold text-[#111110] tracking-tight">Задачи маркетинга</h1>
          <p className="text-[13px] text-[#6b6b66] mt-1">{openCount} открытых · {doneCount} выполнено</p>
        </div>
      ...
      <div className="space-y-2">
        {filtered.length === 0 && <div className="text-center py-16 text-[13px] text-[#8a8a85]">Нет задач. Добавьте первую.</div>}
        {filtered.map(t => {
          const done = t.status === 'done' || t.status === 'cancelled'
          const isOverdue = t.deadline && new Date(t.deadline) < new Date() && !done
          return (
            <div key={t.id} className={`bg-white border rounded-xl p-3 flex items-center gap-3 ${done ? 'opacity-50 border-[#f0f0ec]' : isOverdue ? 'border-red-200' : 'border-[#e4e4e0]'}`}>
              <button onClick={() => setStatus(t, done ? 'todo' : 'done')}
                className={`w-5 h-5 rounded border flex-shrink-0 flex items-center justify-center transition-colors ${done ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-[#c0c0bb] hover:border-emerald-400'}`}>
                {done && '✓'}
              </button>
              <div className="flex-1 min-w-0">
                <p className={`text-[13px] font-medium ${done ? 'line-through text-[#8a8a85]' : 'text-[#111110]'}`}>{t.title}</p>
                <div className="flex flex-wrap gap-1.5 mt-1">
                  ...
                  {t.assignee && <span className="text-[11px] text-[#8a8a85]">@{t.assignee}</span>}
                  {t.deadline && <span className={`text-[11px] ${isOverdue ? 'text-red-500 font-medium' : 'text-[#8a8a85]'}`}>{t.deadline}</span>}
                </div>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6">
            <h2 className="text-[16px] font-semibold text-[#111110] mb-5">{editId ? 'Редактировать задачу' : 'Новая задача'}</h2>
            <div className="space-y-3">
              <div>
                <label className="block text-[12px] font-medium text-[#6b6b66] mb-1">Задача *</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[12px] font-medium text-[#6b6b66] mb-1">Направление</label>
                ...
                <div>
                  <label className="block text-[12px] font-medium text-[#6b6b66] mb-1">Приоритет</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[12px] font-medium text-[#6b6b66] mb-1">Ответственный</label>
                  <input type="text" value={form.assignee ?? ''} onChange={e => setForm(f => ({ ...f, assignee: e.target.value }))}
                    placeholder="Иван"
                    className="w-full border border-[#e4e4e0] rounded-lg px-3 py-2 text-[13px] focus:outline-none focus:ring-2 focus:ring-blue-300" />
                </div>
                <div>
                  <label className="block text-[12px] font-medium text-[#6b6b66] mb-1">Дедлайн</label>
              ...
              {(form.status === 'done') && (
                <div>
                  ...
                  <input type="text" value={form.result ?? ''} onChange={e => setForm(f => ({ ...f, result: e.target.value }))}
