...
function stageColor(o: Order) {
  if (o.shipped) return 'bg-line-soft text-ink-soft border-line'
  ...
  return (
    <div className="min-h-screen bg-canvas pb-20">
      <div className="bg-surface border-b border-line px-4 pt-12 pb-3 lg:pt-6">
        <h1 className="text-[20px] font-semibold text-ink tracking-tight">Мои заказы</h1>
      ...
      <div className="px-4 pt-4 space-y-2 max-w-[760px] mx-auto">
        {!data?.linked && (
          <div className="bg-surface rounded-xl border border-line p-8 text-center">
            <p className="text-[14px] text-ink font-medium">Аккаунт ещё не привязан к вашей компании</p>
            <p className="text-[13px] text-muted mt-1">Обратитесь к вашему менеджеру M-Glass, чтобы открыть доступ к заказам.</p>
          </div>
        )}

        {data?.linked && active.length === 0 && shipped.length === 0 && (
          <div className="bg-surface rounded-xl border border-line p-8 text-center">
            <p className="text-[14px] text-muted">Пока нет заказов</p>
          </div>
        )}

        {active.length > 0 && (
          <>
            <p className="text-[11px] font-semibold uppercase tracking-widest text-muted pt-1">В работе · {active.length}</p>
        ...
        {shipped.length > 0 && (
          <>
            <p className="text-[11px] font-semibold uppercase tracking-widest text-muted pt-3">Отгружены · {shipped.length}</p>
      ...
      <div className="flex items-start justify-between gap-2 mb-2">
        <div className="min-w-0">
          <p className="text-[14px] font-semibold text-ink truncate">
            {o.number}
            {o.clientOrderNumber && <span className="text-muted font-normal"> · ваш № {o.clientOrderNumber}</span>}
          </p>
          <p className="text-[12px] text-muted">от {fmtDate(o.created_at)} · {fmtMoney(o.amount)}</p>
        </div>
        <span className={`text-[11px] font-medium px-2 py-1 rounded-full border whitespace-nowrap flex-shrink-0 ${stageColor(o)}`}>
          {o.stage}
        </span>
      </div>
      {!o.shipped && (
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] text-muted whitespace-nowrap">{o.progressPct}%</span>
          <span className="text-[11px] text-muted whitespace-nowrap">· срок {fmtDate(o.deadline)}</span>
        </div>
      )}
    </div>
  )
