  ...
  return (
    <div className="min-h-screen bg-[#f5f5f3] pb-20">
      ...
      <div className="px-4 pt-4 max-w-[720px] space-y-5">
        ...
        {/* Наклейки — полный цикл */}
        <div className="bg-white rounded-xl border border-[#e4e4e0] px-4 py-4">
          <div className="flex items-center gap-2.5 flex-wrap">
            <h2 className="text-[17px] font-bold text-[#111110]">Наклейки: полный цикл детали</h2>
            <Who color="bg-slate-700">все этапы</Who>
          </div>
          <p className="text-[14px] text-[#16181a] mt-1.5">Наклейка — паспорт детали. По ней сканируем этапы, по ней деталь после закалки находит себя. Термонаклейка держится крепко и в итоге уезжает на изделии к заказчику.</p>
          <Steps items={[
            <><b>Резка.</b> Резчик печатает наклейки (<Tab>🖨 Наклейки</Tab> в заказе) и клеит на деталь. <b>Дополнительно маркером</b> (который стирается) пишет на стекле размер и № заказа — это единственное, что переживёт печь.</>,
            <><b>Обработка.</b> Полировка, сверловка, фацет — на каждом этапе <b>сканируешь наклейку</b> (<Tab>📷 Скан</Tab> или BT-сканер), этап отмечается сам.</>,
            <><b>Перед закалкой — сними наклейку.</b> В печь наклейка НЕ идёт (сгорит). Снятые наклейки клей на <b>доску/стол, сортируя по толщине</b> — сразу видно, что уехало на закалку и что ещё не вернулось. В печи деталь опознаётся по маркеру.</>,
            <><b>После закалки — верни наклейку.</b> Деталь пришла из печи → по маркеру (размер, №) находишь её наклейку на доске и клеишь обратно. Маркер после закалки стирается.</>,
            <><b>ОТК и упаковка.</b> Проверил → <Btn tone="green">Готово</Btn> на упаковке. <b>Та же наклейка остаётся на изделии</b> и уезжает к заказчику — привязка сохраняется до конца.</>,
          ]} />
          <div className="mt-3 bg-slate-100 border border-slate-200 rounded-lg px-3 py-2.5 text-[14px]">
            🏷 <b>Термонаклейки</b> заказываем по размеру нашего принтера (мотки), срок ~2 недели. Клей сильный — держится на стекле от резки до заказчика.
          </div>
        </div>
