...
// Личная вкладка владельца: финансовые обязательства.
// Главный экран — не «сколько должен», а «когда буду свободен».
// Математика — lib/vlad/debtMath.ts (чистый TS, покрыт тестами).
...
export default function VladPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (r.ok) { setObs(d.obligations ?? []); setUnlocked(true) }
    setLoading(false)
  }, [])
  // eslint-disable-next-line react-hooks/set-state-in-effect
  useEffect(() => { load() }, [load])
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f3] pb-24">
      <div className="bg-[#111110] text-white px-4 pt-12 pb-6 lg:pt-8">
        <div className="max-w-[720px] mx-auto">
          <div className="flex items-center justify-between">
            <h1 className="text-[20px] font-bold tracking-tight">Влад · Финансы</h1>
          ...
          {/* Дата свободы — главный экран */}
          {sim && (
            <div className="mt-5">
              <p className="text-[12px] uppercase tracking-widest text-[#9a9a95]">Свобода от долгов</p>
              {sim.months >= 600
                ? <p className="text-[26px] font-bold text-red-400 mt-1">При текущих платежах — недостижима</p>
                : <>
                    <p className="text-[32px] font-bold mt-1">{fmtFreedom(sim.freedomDate)}</p>
                    <p className="text-[14px] text-[#c9c9c4]">через {yearsMonths(sim.months)} · процентов сверху {RUB(sim.totalInterest)}</p>
                  </>}
              {sim.stuck.length > 0 && (
          ...
          <div className="grid grid-cols-2 gap-3 mt-5">
            <div className="bg-[#1d1d1c] rounded-xl p-3">
              <p className="text-[11px] text-[#9a9a95]">Всего долгов</p>
              <p className="text-[18px] font-bold font-mono">{RUB(debt)}</p>
            </div>
            <div className="bg-[#1d1d1c] rounded-xl p-3">
              <p className="text-[11px] text-[#9a9a95]">Платежи в месяц</p>
              <p className="text-[18px] font-bold font-mono">{RUB(load_)}</p>
            </div>
          </div>
        </div>
      </div>

      <div className="max-w-[720px] mx-auto px-4 pt-5 space-y-5">
