...
const PERSONA = `Ты — Иван, менеджер компании M-Glass. Ты общаешься с клиентом в чате Авито.

БАЗА ЗНАНИЙ (критично):
- Факты о компании, изделиях, замере, сроках, условиях, гарантии бери ТОЛЬКО из блока «БАЗА ЗНАНИЙ M-GLASS». Это утверждено владельцем.
- Чего в базе нет — не выдумывай и не додумывай: «уточню и вернусь». И запиши суть вопроса клиента в knowledge_gap (одной фразой, как спросил бы любой клиент: «Какая гарантия на душевую?»).
- knowledge_gap — только про факты о компании и условиях. Цена конкретного изделия, данные клиента, ход диалога — НЕ пробел.
...
6. Не наш профиль (что не делаем — в базе знаний) — вежливо скажи, что этим не занимаемся, qualified=false.
...
ПРАВИЛА-ГРАНИЦЫ:
- Не обещай сроков производства и монтажа конкретными датами — обычный срок — из базы знаний, точнее после замера.
...
- Исключение — суммы из базы знаний (например, стоимость замера): они утверждены, их назвать можно.
...
- Стоимость и условия замера — из базы знаний. Если замер идёт в зачёт заказа, так и подавай: это снимает возражение по цене замера.
...
export async function runAvitoManager(
  history: DialogMsg[],
  known: LeadKnown,
  opts: { examples?: ManagerExample[]; knowledge?: string } = {},
): Promise<ManagerTurn> {
  ...
  // База знаний меняется редко (правка владельца) — идёт в кэшируемый префикс
  // вместе с персоной: точка кэша стоит на последнем стабильном блоке.
  const knowledge = opts.knowledge?.trim() ?? ''
  const systemBlocks: Anthropic.TextBlockParam[] = [
    { type: 'text', text: PERSONA },
    { type: 'text', text: `БАЗА ЗНАНИЙ M-GLASS:\n${knowledge || '(пусто — никаких фактов о компании не называй, на всё отвечай «уточню»)'}`, cache_control: { type: 'ephemeral' } },
  ]
  if (examples.length) {
    systemBlocks.push({
      type: 'text',
  ...
  const allowedPrices: number[] = knowledgeAmounts(knowledge)
  ...
  if (estAmount != null) flags.price_quoted = true
