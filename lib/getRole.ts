...
export const ROLE_ALLOWED: Record<Role, string[]> = {
  ...
  manager: [
    ...
    '/kp',
    '/contracts',
    // «Мой день» — что требует действия сегодня. Появился вместе со сворачиванием
    // меню: список из двадцати экранов не отвечал на вопрос «за что браться».
    '/my-day',
    '/calculator/quick',
    // «Расчёт B2B» — B2B-калькулятор в контуре M-Glass: без выбора клиента
    // и срока производства, с панелью быстрого расчёта справа. Тонкий
    // маршрут поверх того же экрана, не форк. Права открыты до выката,
    // чтобы менеджер не увидел «нет доступа» в день появления пункта.
    // «Расчёт» — модель + габариты проёма → себестоимость из реального состава
    // комплекта и раскроя, дальше маржа/налог/монтаж как в быстром расчёте.
    '/calculator/build',
    '/calculator/b2b-mglass',
    // Зеркало, душевая, лофт-перегородка, лестничное ограждение и скан
    // дизайн-проекта убраны из доступа 03.09.2026 по решению владельца:
    // «оно сейчас не отображает честную картину». Конкретно у душевой
    // фурнитура в бюджетном тире бралась не из справочника цен, а из
    // таблицы чисел, вписанных руками, — и разошлась с реальностью.
    // Код экранов на месте, вернуть = вернуть строки сюда и пункты в меню.
    // У менеджера остаются «Быстрый расчёт» и «Визуализатор 3D».
    '/configurator',
    '/calculations',
