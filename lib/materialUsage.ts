...
// Доля крупного остатка, которая реально возвращается в производство. ОДИН
// коэффициент на всю систему: им считает калькулятор (авторасход в позициях) и
// им же объясняется отход на экране экономики. До 15.09.2026 здесь было 0.7, а в
// калькуляторе 0.85 — и один заказ показывал две разные цифры материала.
//
// С 16.09.2026 — 1: заказ платит за детали, рез и полосы, а кусок от 400×800 мм
// (REMNANT_MIN_*) не отход, а приход на стеллаж — его оплачивает тот заказ, который
// его возьмёт. Раскрой считается по каждому заказу отдельно, поэтому «остаток» в
// модели — это почти весь лист: при 0.85 одна деталь 0,8 м² из листа 7,2 м²
// списывала на заказ ещё 0,96 м² (заказ #5479, замечание владельца 16.09).
// Отход сверх нетто по 385 заказам за 90 дней: было 21,9%, стало 12,9% — это рез и
// полосы. Обратная сторона: если остатки со стеллажа не пойдут в дело, разница
// вылезет списанием склада, а не в себестоимости заказа. Смотреть по журналу
// листов (Э6.5), потом завести процент по материалу.
export const DEFAULT_REUSE_RATE = 1
 ...
 * DEFAULT_REUSE_RATE = 1). Иначе мелкий заказ бумажно «съедает» целый лист: остаток
 * от 400×800 мм — приход на стеллаж, а не отход этого заказа.
 */
